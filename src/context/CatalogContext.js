...
const CatalogContext = createContext({
  routes: [], metadata: null, publishedAt: null, status: 'loading', source: null, error: null, refresh: async () => {},
});

export function CatalogProvider({ children }) {
  const { availability } = useNetwork();
  const [catalog, setCatalog] = useState({ routes: [], metadata: null, publishedAt: null, status: 'loading', source: null, error: null });

  const loadCache = useCallback(async (error = null) => {
    const cached = await getCachedRouteDataset();
    setCatalog(cached
      ? { routes: cached.routes, metadata: cached.metadata, publishedAt: cached.validatedAt, status: 'ready', source: 'cache', error }
      : { routes: [], metadata: null, publishedAt: null, status: 'error', source: null, error: error || 'No existe una caché publicada íntegra.' });
  }, []);
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setCatalog({ routes: published.routes, metadata: published.metadata, publishedAt: published.publishedAt, status: 'ready', source: 'firestore', error: null });
    } catch {
