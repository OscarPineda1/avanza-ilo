...
export default function LocationPermissionScreen({ navigation }) {
    ...
    const { setLocationEnabled } = useLocationConsent();
    const enable = async () => {
        const { status } = await Location.requestForegroundPermissionsAsync();
        if (status === 'granted') {
            setLocationEnabled(true);
            navigation.navigate('NearbyStops', { useLocation: true });
            return;
        }
        setLocationEnabled(false);
        setDenied(true);
    };
    return <SafeAreaView style={styles.safeArea}><View style={styles.container}><View style={styles.mapIcon}><Ionicons name="location" size={44} color={theme.colors.primary} /></View><Text style={styles.title}>Activemos tu ubicación</Text><Text style={styles.message}>La ubicación es opcional y solo ayuda a sugerir referencias cercanas del recorrido. No muestra dónde están las unidades ni se guarda.</Text><View style={styles.steps}><Text style={styles.step}>1. Toca «Permitir ubicación»</Text><Text style={styles.step}>2. Elige «Mientras uso la app»</Text></View>{denied && <Text style={styles.denied}>No se concedió el permiso. Elige manualmente dónde esperar.</Text>}<AppButton label="Permitir ubicación" onPress={enable} style={styles.button} /><AppButton label="Elegir dónde esperar" variant="text" onPress={() => navigation.navigate('NearbyStops')} /></View></SafeAreaView>;
}
