    ...
    return (
        <SafeAreaView style={styles.safeArea}>
            ...
            <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
                {!query.trim() ? <View style={styles.help}><Ionicons name="bulb-outline" size={19} color={theme.colors.primary} /><Text style={styles.helpText}>Catálogo piloto completo. Busca por código, nombre, origen o destino.</Text></View> : null}
                {results.length ? <><Text style={styles.count}>{results.length} {results.length === 1 ? 'resultado' : 'resultados'}</Text>{results.map((item) => <RouteListItem key={item.id} route={item} onPress={() => navigation.navigate(item.coordinates?.length ? 'RouteDetails' : 'Status', item.coordinates?.length ? { routeName: item.nombre, sequenceId: item.defaultSequenceId } : { type: 'offline', routeName: item.nombre })} />)}</> : <EmptyState title="No encontramos resultados" message={`No hay resultados para “${query}”. Prueba buscando por el nombre de la ruta, origen o destino.`} actionLabel="Ver todas las rutas" onAction={() => navigation.navigate('MainTabs', { screen: 'ExploreRoutes' })} />}
            </ScrollView>
        </SafeAreaView>
    );
}
