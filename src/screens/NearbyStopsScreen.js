...
export default function NearbyStopsScreen({ navigation, route }) {
    ...
    const allStops = useMemo(() => routes.filter((item) => item.pilot && item.available).flatMap((item) => item.sequences.flatMap((sequence) => sequence.stops.map((stop) => ({ ...stop, color: item.color })))), [routes]);
