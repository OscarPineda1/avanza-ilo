...
import { canTravelInSequence } from '../services/eta';
...
export default function StopSelectionScreen({ navigation, route }) {
    ...
    const destination = selectedSequence?.stops.at(-1);
    const selectionUnavailable = Boolean(
        selected &&
        destination &&
        !canTravelInSequence(selectedSequence, selected.id, destination.id)
    );
